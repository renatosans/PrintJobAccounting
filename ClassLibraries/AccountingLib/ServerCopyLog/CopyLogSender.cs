...
{
    public class CopyLogSender
    {
        private List<CopiedDocument> copiedDocumentList;
        ...
        public CopyLogSender(String serviceUrl, IListener listener)
        {
            ServicePointManager.Expect100Continue = false;

            this.copiedDocumentList = new List<CopiedDocument>();
        ...
        private Boolean TrySend(int timeout)
        {
            ...
            if (listener != null) listener.NotifyObject("Enviando pacote de logs de cópia");
            ...
            try
            {
                Byte[] serializedObject = ObjectSerializer.SerializeObjectToArray(copiedDocumentList);
                ...
                request = (HttpWebRequest)WebRequest.Create(serviceUrl + "?action=LogCopiedDocuments");
                request.Method = "POST";
